...
const CarouselItem: React.FC<{ children: ReactNode; className?: string }> = ({
  children,
  className = '',
}) => <div className={`snap-start flex-shrink-0 ${className}`}>{children}</div>;

const CarouselContent = forwardRef<HTMLDivElement, { children: ReactNode; className?: string }>(
    ...
    >
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <Badge className="mb-4 bg-[hsl(200_75%_55%)] text-white border-0">Преимущества</Badge>
          <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-[hsl(200_80%_70%)] to-[hsl(210_90%_30%)] bg-clip-text text-transparent">
          ...
          <CarouselContent className="gap-6">
            {advantages.map(({ icon: IconComponent, title, description }, idx) => (
              <CarouselItem key={idx} className="pl-2 md:pl-4 md:basis-1/3">
                <Card className="group relative overflow-hidden bg-gradient-to-br from-[hsl(200_60%_20%)] to-[hsl(210_60%_15%)] border-0 shadow-lg hover:shadow-[hsl(200_80%_70%)] transition-all duration-500 hover:-translate-y-2">
                  <CardContent>
                    <div className="flex flex-col items-center text-center space-y-4 h-full">
                      <div className="p-3 bg-gradient-to-r from-[hsl(200_75%_55%)] to-[hsl(210_75%_35%)] rounded-xl group-hover:scale-110 transition-transform duration-300 shadow-lg">
                        <IconComponent className="h-6 w-6 text-white" />
                      </div>
                      <h3 className="text-lg font-semibold text-white group-hover:text-[hsl(200_80%_70%)] transition-colors">
                        {title}
                      </h3>
                      <p className="text-sm text-gray-300 leading-relaxed flex-1">{description}</p>
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-br from-[hsl(200_75%_55%)]/10 to-[hsl(210_75%_35%)]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                  </CardContent>
                </Card>
              </CarouselItem>
            ))}
        ...
        <div className="text-center mt-16">
          <div className="inline-block p-8 bg-gradient-card rounded-3xl shadow-elegant">
            <h3 className="text-2xl font-bold mb-4 text-white">Готовы стать частью инновации?</h3>
            <p className="text-gray-300 mb-6 max-w-md">
              Присоединяйтесь к нашей семье партнеров и создайте успешный бизнес в уникальной нише
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button className="px-6 py-3 bg-gradient-to-r from-[hsl(200_75%_55%)] to-[hsl(210_75%_35%)] text-white rounded-xl font-medium shadow-lg hover:shadow-[hsl(200_80%_70%)] transition-all duration-300 hover:scale-105">
                Узнать условия
              </button>
